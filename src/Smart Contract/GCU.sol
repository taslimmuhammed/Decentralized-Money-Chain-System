...
contract GCU {
    ...
    IERC20 token = IERC20(0x39831560d4f5B099532086Cd4E58242C362Be0d8);
    uint256 reservoir;
    uint256 unitLimit;
    uint256 totalSupply;
    address[2] Lot;
    ...
    function chnageOwner(address _newOwner) public {
        require(msg.sender == owner, "only owner can nominate new Owner");
        owner = _newOwner;
    }

    function checkUserLimit(bool _active) public view returns (bool) {
        if (_active == true) {
            if (unitCount[msg.sender] <= 90) return true;
            else return false;
        } else {
            if (unitCount[msg.sender] < 1) return true;
            else return false;
        }
    }
    ...
    function signIn(address _friend, bool _active) public returns (bool) {
        ...
        myRefferals[_friend].push();
        return true;
    }

    function handleReferal() public {
    ...
    function handleLot(uint256 x, bool _active) private {
        ...
        if (Lot[0] == 0x0000000000000000000000000000000000000000) {
            Lot[0] = msg.sender;
            return;
        } else if (Lot[1] == 0x0000000000000000000000000000000000000000) {
            Lot[1] = msg.sender;
            ...
            if (x % 2 == 1) {
                if (_active == true)
                    unitBalance[msg.sender] = unitBalance[msg.sender].add(120);
                else token.transfer(Lot[0], 12000000);
                emit LotResult(Lot[0], Lot[1], 12000000);
            } else {
                if (_active == true)
                    unitBalance[Lot[1]] = unitBalance[Lot[1]].add(120);
    ...
    function buyUnitToken() public loggedIn {
        bool _active = true;
        if (active[msg.sender] == 1) _active = false;
        require(checkUserLimit(_active), "Maximum user limit reached");
